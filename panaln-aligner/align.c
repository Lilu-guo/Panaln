...
int index_bwt(const char* fastaFname, const char* extSAFname) {
	...
	printf("build fmindex seqLen:%lld \n",seqLen);
	free(annFname);
	free(refFname);
	return 0;
}
...
int align_reads(char *fastaFname, char *readsFname, char *alnsFname, aln_params_t *params)
{
	...
	else
	{
		...
		{
#ifdef FM
			align_reads_inexact_fm(fm, reads, sa_intv_table, params, alnsFname);
#else
#endif
		}
	}
	...
	free_reads(reads);
	if (sa_intv_table)
		free_sa_interval_list(sa_intv_table);
	free(bwtFname);
	free(preFname);
	return 0;
}
...
{
	if (fread(&intv_list->size, sizeof(int), 1, saFile) < 1)
		load_sa_intervals_error();
	for (int j = 0; j < intv_list->size; j++)
	{
		sa_intv_t *intv = (sa_intv_t *)calloc(1, sizeof(sa_intv_t));
		if (fread(&intv->L, sizeof(bwtint_t), 1, saFile) < 1)
			load_sa_intervals_error();
		if (fread(&intv->U, sizeof(bwtint_t), 1, saFile) < 1)
			load_sa_intervals_error();
		if (j == 0)
		{
			intv_list->first_intv = intv;
...
void free_alignments(alns_t *alns)
{
	...
	{
		if (alns->entries[i].aln_path)
			free(alns->entries[i].aln_path);
	}
	free(alns->entries);
	free(alns);
}
...
alns_t *alnsf2alns(int *n_alns, char *alnFname)
{
	...
	while (!feof(alnFile))
	{
		...
		for (int i = 0; i < read_alns->num_entries; i++)
		{
			...
			{
				if (fscanf(alnFile, "%c ", &(aln->aln_path[j])) != 1)
					load_alns_error(alnFname);
			}
			if (fscanf(alnFile, "\n") < 0)
				load_alns_error(alnFname);
...
alns_t *alnsf2alns_bin(int *n_alns, char *alnFname)
{
	...
	while (!feof(alnFile))
	{
		...
		for (int i = 0; i < read_alns->num_entries; i++)
		{
			...
			if (fread(&(aln->score), sizeof(int), 1, alnFile) < 1)
				load_alns_error(alnFname);
			if (fread(&(aln->L), sizeof(unsigned long long int), 1, alnFile) < 1)
				load_alns_error(alnFname);
			if (fread(&(aln->U), sizeof(unsigned long long int), 1, alnFile) < 1)
				load_alns_error(alnFname);
			if (fread(&(aln->num_mm), sizeof(int), 1, alnFile) < 1)
				load_alns_error(alnFname);
			if (fread(&(aln->num_gapo), sizeof(int), 1, alnFile) < 1)
				load_alns_error(alnFname);
			if (fread(&(aln->num_gape), sizeof(int), 1, alnFile) < 1)
				load_alns_error(alnFname);
			if (fread(&(aln->aln_length), sizeof(int), 1, alnFile) < 1)
				load_alns_error(alnFname);
			aln->aln_path = (char *)malloc(aln->aln_length * sizeof(char));
			int state_pairs;
			if (fread(&state_pairs, sizeof(int), 1, alnFile) < 1)
				load_alns_error(alnFname);
			int state_pair;
			int path_idx = 0;
			for (int j = 0; j < state_pairs; j++)
			{
				if (fread(&state_pair, sizeof(int), 1, alnFile) < 1)
					load_alns_error(alnFname);
...
{
	if (read->aln_top1_count == 0)
		return 23;
	if (is_multiref)
	{
		if (read->aln_top1_count > read->num_mref_pos)
			return 0;
	}
	else
	{
		if (read->aln_top1_count > (read->ref_pos_r - read->ref_pos_l + 1))
			return 0;
	}
	if (read->num_mm == max_mm)
		return 25;
	if (read->aln_top2_count == 0)
		return 37;
