...
{	
	...
	int step =this->D;
	SAL=new InArray((n+1)/step+2,blog(n+1)+1);
	fm_int i=0, j=0;
	for(i=0; i<=n; i++){
		if(i%step ==0){
			SAL->SetValue(j++,SA[i]);                
...
int ABS_FM::loadIdx()
{
	...
	string fnewD2 = "/home/lab/gll/panaln-git/panaln-aligner/FM/newD.idx.256";
	loadkit s2(fnewD2.c_str());
	newD->load(s2);
	s2.close();

	string flroot2 = "/home/lab/gll/panaln-git/panaln-aligner/FM/lroot.idx.256";
	loadkit s3(flroot2.c_str());
	LoadWTTree(s3, lroot);
	s3.close();

	string frroot2 = "/home/lab/gll/panaln-git/panaln-aligner/FM/rroot.idx.256";
	loadkit s4(frroot2.c_str());
	LoadWTTree(s4, rroot);
	s4.close();

	string fsal2 = "/home/lab/gll/panaln-git/panaln-aligner/FM/sal.idx.256";
