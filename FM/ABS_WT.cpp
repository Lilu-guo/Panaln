...
ABS_FM::ABS_FM(const char * filename,int block_size,int r,int D) //存索引、读索引都会用到
{
	// char* newfilename = (char*)malloc(strlen(filename)+4);
	// sprintf(newfilename, "../%s", filename); //由于是FM目录,需要在路径上增加一层

	this->block_size = block_size;
	this->r=r;
	this->D =D;
	this->T=NULL;
	// T = Getfile(newfilename);    
	// this->filename=(char *)newfilename;
	T = Getfile(filename);                                                                        //扫描本文，构建C表
	this->filename=(char *)filename; //保存参考序列名，为索引idx3动态命令用，2025.3.23
	Inittable();
}
...
fm_int ABS_FM::FM_Lookup(fm_int i)          //下标采样
{
	...
	// cout<<"i:"<<i<<" Di:"<<newd->getD(i)<<"  Di-1:"<<newd->getD(i-1)<<"  Di+1:"<<newd->getD(i+1)<<endl;
	while(newd->getD(i)!=1)                //值采样
	// while(i%D !=0)
	{
		// cout<<newD->getD(i)<<"  ";
	...
	// cout<<"sal:"<<sal<<endl;
	/* cout<<"D:"<<D<<" step:"<<step<<" r:"<<r<<endl;
	int j=5;
	for(int k=-j; k<j; k++){ //打印附件的sal，看是否有正确的
		// cout<<" sa"<<k<<":"<<SAL->GetValue(i/D +k); //调下标采样
		cout<<" sa"<<k<<":"<<SAL->GetValue(r +k); //调值采样
	}
	cout<<endl; */
	return (sal*D+step)%n;
}
...
// unsigned char ABS_FM::L(fm_int i) //原始的L函数
// {
// 	BitMap * r = root;
// 	int bit =0;
// 	fm_int rank = 0;
	
// 	while(r->Left()) //不是叶子一直找
// 	{
// 		rank = r->Rank(i,bit); //bit为传出0或1
// 		if(bit ==1)
// 		{
// 			i = rank -1;
// 			r=r->Right(); //跳到右孩子
...
// 		}
// 	}
// 	return r->Label(); //取节点label

// }

unsigned char ABS_FM::myL(fm_int i) //为Occ1函数的一部分
{
	fm_int ii =i;
	BitMap *r;
	int bit =0;
...
void ABS_FM::mycodetable(){ //codeTable给的是gray码
	...
	// memset(C, 0, 16 * 4); //改为在loadIdx中直接赋值了. 2025.4.6
	...
	// //genome + 124w (2024.6.5, bioinf论文用, 2025.3.23注释)
	...
	//改为在loadIdx中直接赋值了. 2025.4.6
	// C[0]=0;          C[1]=233888;    C[2]=164277123;  C[3]=164465515;  C[4]=283549815;  C[5]=283732617;  C[6]=283742066;  C[7]=284512743; //C表 gray序（不含结尾处的$）
	// C[8]=403597043; C[9]=403785435; C[10]=403792795; C[11]=403802244; C[12]=404572921; C[13]=404580281; C[14]=404741655; C[15]=568784890;
	...
	// //300w
	// C[0]=0;          C[1]=3004308;    C[2]=2353986633;  C[3]=2357204015;  C[4]=3992316145;  C[5]=3995888377;  C[6]=3995888831;  C[7]=4007059372; //C表 gray序（不含结尾处的$）
	// C[8]=5642171502; C[9]=5645388884; C[10]=5645389383; C[11]=5645389837; C[12]=5656560378; C[13]=5656560877; C[14]=5659723575; C[15]=8010705900;
}
...
{	
	...
	newd->constructrank(256,16); //值采样标记数组
	if((n)%step!=0){ //检查这里是否应为n-1???
		RankL->SetValue((n)/step+1,newd->rank2(0)); //检查这里是否应为n-1???
	}
	for(i=0; i<=n; i++)
	{
		if(SA[i]%step==0){
			RankL->SetValue(SA[i]/step,newd->rank2(i));   //逆SA采样
		}
	}
	string prefix(this->filename);
	// cout<<prefix<<endl; getchar();
	prefix = prefix.substr(0, prefix.find('.')); //得到basename
	// cout<<"prefix: "<<prefix<<endl; getchar();

	cout<<"Construct Sal ..."<<endl;
	// string fsal = "sal.idx4"; //保存SA采样
	string fsal = prefix+".1.idx"; //2024.6.4，支持250bp的读长做变异发现
	savekit s4(fsal.c_str());
	SAL->write(s4); 
	SAL->~InArray();
	SAL=NULL;
	...
	// string frankl = "rankl.idx4"; //保存逆SA采样
	...
	BWT(T,SA,bwt,n);
	// cout<<"sa0_index:"<<sa0_index<<" n:"<<n<<endl;
	string finfo = prefix+".7.idx"; //放alphaSize值,c表,sa0值,D采样,n值
	ofstream inf(finfo);
	...
	newD->constructrank(1024,16); //根上的位向量 U
	// cout<<"Dmap space: "<<newD->GetMemorySize()/1024/1024<<" MB"<<endl;
	// string fnewD = "newD.idx4"; //写索引2
	...
	// cout<<"sa0: "<<this->sa0_index<<"  D:"<<this->D<<"  n:"<<this->n<<"  alphabetSize: "<<this->alphabetsize<<endl;
...
// int ABS_FM::BuildTree()                                    //值采样，测试索引构建时间，2023-4-10 (Bioinf投稿论文用，2025.3.23)
// {	
// 	saidx64_t *SA = new saidx64_t[n+1];
// 	SA[0] = n;
// 	cout<<"build tree n:"<<n<<"  D:"<<this->D<<endl;       //这里的D为32，没问题
...
// 		}
// 	}
// 	cout<<"construct sal.idx ..."<<endl;
// 	// string fsal = "sal.idx4"; //保存SA采样
// 	string fsal = "sal.gw250.idx"; //2024.6.4，支持250bp的读长做变异发现
// 	savekit s4(fsal.c_str());
// 	SAL->write(s4); 
// 	SAL->~InArray();
// 	SAL=NULL;
// 	s4.close();
// 	cout<<"construct rankl.idx ..."<<endl;
// 	// string frankl = "rankl.idx4"; //保存逆SA采样
// 	string frankl = "rankl.gw250.idx";
// 	savekit s5(frankl.c_str());
// 	RankL->write(s5);
// 	RankL->~InArray();
// 	RankL=NULL;
...
// 	bwt = new unsigned char[n+1];                                                                                  //用于存放bwt串
// 	BWT(T,SA,bwt,n);
// 	cout<<"sa0_index:"<<sa0_index<<" n:"<<n<<endl;                                                                                               //生成bwt（传入n，但bwt的长度为n+1）
// 	delete [] T; //释放T
// 	T=NULL;
// 	delete [] SA; //释放SA
// 	SA=NULL;
...
int ABS_FM::loadIdx()
{
	...
	// cout<<"loadIdx..."<<endl;
	// string prefix=this->filename; //2025.3.23
	// prefix =prefix +".ref"; //2025.3.23
	// cout<<"prefix:"<<prefix<<endl; /* getchar(); */
	...
	// string fsal2 = prefix+"sal.idx150";
	string fsal2 = prefix+".1.idx";
	// string fsal2 = "/home/lab/gll/panaln/mg-aligner/sal.gw250.idx";
	loadkit s1(fsal2.c_str());
	this->SAL = new InArray();
	SAL->load(s1); //载入后，用SAL指针指向（ABS_FM属性）
	s1.close();

	// cout<<"load Rankl ..."<<endl;
	// string rankl2 = prefix+"rankl.idx150";
	string rankl2 = prefix+".2.idx";
	// string rankl2 = "/home/lab/gll/panaln/mg-aligner/rankl.gw250.idx";
	...
	// cout<<"load D ..."<<endl;
	// newD = new Dmap(6971866259); //这是原本的2024.4.18
	this->newd = new Dmap(); //这里是否需要指定大小???
	// string fnewD2 = "/home/lab/gll/panaln/mg-aligner/FM/newD.idx.256"; //blockSize大小256,128,64,32
	// string fnewD2 = prefix+"newD.idx150";
	string fnewD2 = prefix+".3.idx";
	// string fnewD2 = "/home/lab/gll/panaln/mg-aligner/newD.gw250.idx"; //2024.6.4，支持250bp的读长做变异发现
	loadkit s3(fnewD2.c_str());
	newd->load(s3); //载入后，用newD指针指向（ABS_FM属性）. 值采样的标记数组
	s3.close();

	// cout<<"load U ..."<<endl;
	// string fnewd2 = prefix+"newd.idx150";
	string fnewd2 = prefix+".4.idx";
	// string fnewd2 = "/home/lab/gll/panaln/mg-aligner/newd.gw250.idx";
	loadkit s4(fnewd2.c_str());
	this->newD = new Dmap(); //根上的位向量 U
	newD->load(s4);
	s4.close();

	// cout<<"load Ltree ..."<<endl;
	// string flroot2 = "/home/lab/gll/panaln/mg-aligner/FM/lroot.idx.256";
	// string flroot2 = prefix+"lroot.idx150";
	string flroot2 = prefix+".5.idx";
	// string flroot2 = "/home/lab/gll/panaln/mg-aligner/lroot.gw250.idx";
	loadkit s5(flroot2.c_str());
	LoadWTTree(s5, lroot); //载入后，用lroot指针指向（ABS_FM属性）
	s5.close();

	// cout<<"load Rtree ..."<<endl;
	// string frroot2 = "/home/lab/gll/panaln/mg-aligner/FM/rroot.idx.256";
	// string frroot2 = prefix+"rroot.idx150";
	string frroot2 = prefix+".6.idx";
	// string frroot2 = "/home/lab/gll/panaln/mg-aligner/rroot.gw250.idx";
	loadkit s6(frroot2.c_str());
	LoadWTTree(s6, rroot); //载入后，用rroot指针指向（ABS_FM属性）
	s6.close();

	// //genome.fa + snp144Comm + w参数：124
	// this->sa0_index =238834933;
	// this->D =32; //sa采样
	// this->n =568784890; //给ABS_FM属性赋值
	...
	this->C =new fm_int[this->alphabetsize +1]; //多1是，C表的尾部为总数
    for(int i=0; i<alphabetsize+1; i++){
        inf >> this->C[i];                
    }
    inf.close();

	// cout<<this->sa0_index<<endl;
	// cout<<this->D<<endl;
	// cout<<this->n<<endl;
	// cout<<this->alphabetsize<<endl;
	// for(int i=0; i<alphabetsize+1; i++){
    //     cout<<this->C[i]<<endl;                
    // }
	...
	// //genome.fa + snp144Comm + w参数：300
	// this->sa0_index =3370608629;
	// this->D =32; //sa采样
	// this->n =8010705900; //给ABS_FM属性赋值
	// this->alphabetsize =15; //除去N
	// this->C =new fm_int[alphabetsize+1]; //多1是，C表的尾部为总数
