...
void print_bubble(string chr, string schr, string bubble_filename, string data_filename, char *chromosome, long long &indel_count, long long start, int &flag, long long &total_indel_number, long long &low_end_indel_number, pars_t *pars)
{
	...
	if (ext.good())
	{
		while (ext >> pos >> indel_ref >> indel_alt >> occ)
		{
			...
			total_indel_number++;
			bubble << ">"
				   << "bubble" << indel_count << " " << chr << " " << get_max(pos - pars->window_size, 1) << endl;
			...
			if (indel_ref[0] != '.')
				ref_len = indel_ref.length();
			else
				ref_len = 0;
			if (indel_alt[0] != '.')
				alt_len = indel_alt.length();
			else
				alt_len = 0;
			data << chr << endl;
...
void acgt_bubble(string fasta_filename, string acgt_bubble_filename, string data_filename, pars_t *pars)
{
	ifstream fasta;
	ofstream acgt_bubble;
	string tmp;
	fasta.open(fasta_filename.c_str());
	acgt_bubble.open(acgt_bubble_filename.c_str());
	while (getline(fasta, tmp))
	{
		acgt_bubble << tmp << endl;
	}
	fasta.clear();
	...
	while (getline(fasta, line))
	{
		if (line.length() >= 1 && line[0] == '>')
		{
			...
			{
				print_bubble(chr, schr, acgt_bubble_filename, data_filename, chromosome, indel_count, start, flag, total_indel_number, low_end_indel_number, pars);
			}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "w:i:a:")) >= 0)
	{
		...
		case 'w':
			...
			break;
		case 'i':
			pars->min_occ_specified = 1;
			pars->min_occ = atoi(optarg);
			break;
		case 'a':
			pars->max_occ_specified = 1;
	...
	// string acgt_filename = argv[optind+4];
	...
	// acgt_bubble(fasta_filename, acgt_filename, data_filename, pars);
